---
title: Ch01 K8s 1.25 deployment
date: 2022-11-22
tags:
 - RabbitMQ
 - MQ
...
```sh
curl -L "https://github.com/rabbitmq/cluster-operator/releases/latest/download/cluster-operator.yml" > cluster-operator.yml
```
...
```sh
kubectl get all -n rabbitmq-system    #查看创建的operator资源
kubectl get customresourcedefinitions.apiextensions.k8s.io | grep rabbitmq    #查看rabbitmq自定义资源
```
...
使用nfs：https://github.com/0YHR0/InformationSystem/blob/master/env/config/nfs_setup/NFS%20deployment.md
...
spec:
  ...
  nfs:
    path: /mnt/nfs/rabbitmq/1
    server: 124.220.35.58
---
apiVersion: v1
kind: PersistentVolume
metadata:
...
spec:
  ...
  persistentVolumeReclaimPolicy: Retain
  nfs:
    path: /mnt/nfs/rabbitmq/9
    server: 124.220.35.58
```



rabbitmq-depolyment.yaml
...
spec:
  replicas: 2
  service:
    type: NodePort   #通过NodePort暴露端口，可用在集群外进行访问
    #type: LoadBalancer
    #type: ClusterIP  K8s内部集群调用
  override:
    statefulSet:
      spec:
        volumeClaimTemplates:
  ...
  rabbitmq:
    additionalConfig: |
      ...
      default_pass = 123456      #配置账号密码
```



![image-20221128222410755](https://markdown-1301334775.cos.eu-frankfurt.myqcloud.com/image-20221128222410755.png)

+ 第一组端口是AMQP协议连接端口
+ 第二组端口是UI，可以浏览器访问
+ 第三组端口是prometheus获取监控信息

![image-20221128222900795](https://markdown-1301334775.cos.eu-frankfurt.myqcloud.com/image-20221128222900795.png)